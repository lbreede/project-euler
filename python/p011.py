...
def gen_grid(lines):
    grid_list = []
    for line in lines:
        grid_list.append([int(val) for val in line.split()])
    return grid_list
...
def process_cols(grid, value, offset=[0, 0, 0, 0]):
    ...
    Args:
        grid (list[int]): A nested list of columns and rows.
        value (int): The parsed value gets processes by all function, yielding
            the final result at the very end.
        offset (list[int]): A lookup offset, enabling this function to also
            process diagonals. Recommended patterns are 0,0,0,0 (default),
            0,1,2,3 and 3,2,1,0

    Returns: The integer value compared with itself on every iteration and
        increased if needed. This will always yield the largest possible number
        per function.
    """
